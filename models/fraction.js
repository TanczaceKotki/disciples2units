...
var Fraction = function () {
    ...
    this.add = function (req, filename) {
        ...
        that.source.root.fraction.push(
            xml.blankFraction(req.body.title, req.body.desc, img)
        );
        that.save();
    };
    ...
    this.addUnit = function (req) {
        for (var i = 0; i < that.details.length; ++i) {
            if (that.details[i].title === req.body.fraction) {
                that.source.root.fraction[i][req.body.profession].unit.push({'label': ' '});
                break;
            }
        }
        that.save();
